...
export function LessonScreen({ route, navigation }: NativeStackScreenProps<CoursesStackParamList, 'Lesson'>) {
  ...
  if (state.loading || !state.data || locked) return <View style={[s.center, { paddingTop: insets.top + 24 }]}>
    {state.loading ? <><ActivityIndicator /><Text style={s.body}>Preparando tu lección…</Text></> : <>
  ...
  return <KeyboardAvoidingView style={[s.page, { paddingTop: insets.top }]} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
    <View style={local.header}>
      <View style={local.row}>
        <Pressable disabled={state.busy} onPress={back} accessibilityRole="button" accessibilityLabel="Volver" style={local.back}><View style={local.chevron} /></Pressable>
    ...
    <ScrollView ref={scroll} keyboardShouldPersistTaps="handled" contentContainerStyle={[s.content, { paddingBottom: Math.max(24, insets.bottom + 12) }]}>
...
const local = StyleSheet.create({
  ...
  back: { width: 40, minHeight: 44, justifyContent: 'center', paddingLeft: 5 },
  chevron: { width: 14, height: 14, borderLeftWidth: 3, borderBottomWidth: 3, borderColor: '#0062E9', transform: [{ rotate: '45deg' }] },
  topic: { flex: 1, fontSize: 16, lineHeight: 21, fontWeight: '700', color: '#101B4D' },
});
